...
const UserDashboard = () => {
  ...
  // Helper functions - defined after state but before main logic
  ...
  // Derived state - defined after helper functions
  ...
  // IMPORTANT: Define hasActiveLoans AFTER loans is available but before any functions that use it
  ...
  // Event handlers - defined after derived state
  const handleLogout = () => {
    dispatch(logout());
  };
  ...
  const handleDeleteAccount = async () => {
    const hasActiveLoansForDelete = loans.some((loan) =>
      ["pending", "approved", "partial"].includes(loan.status),
    );

    if (hasActiveLoansForDelete) {
  ...
  // Render functions - defined after all logic
  const renderHomeTab = () => (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-2xl p-6 text-white shadow-lg">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-xl font-semibold mb-3">Make Loan Repayment</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <p className="text-sm text-gray-600">Get quick cash for your needs</p>
        </button>
          ...
          <div className="space-y-4">
            {loans.slice(0, 3).map((loan) => {
              const totalRepayment =
                loan.totalAmount ||
                loan.amount + (loan.amount * (loan.interestRate || 10)) / 100;
              const remainingBalance = totalRepayment - (loan.amountPaid || 0);
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm">
        ...
        <div className="mb-4 p-3 bg-blue-50 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Current Status:</strong> {user?.phoneNumbers?.length || 0}{" "}
  ...
  const renderSettingsTab = () => {
    // Local variables for settings tab only
    const hasActiveLoansForSettings = loans.some((loan) =>
      ["pending", "approved", "partial"].includes(loan.status),
    );
            ...
            <div className="border-t pt-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium">{loans.length}</span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium">
                    {
                      loans.filter((l) =>
                        ["pending", "approved", "partial"].includes(l.status),
                      ).length
                    }
                  </span>
                </div>
                  ...
                  <div className="flex-1">
                    ...
                    {!canDeleteAccount && (
                      <div className="mt-3 space-y-2">
                        {hasActiveLoansForSettings && (
                          <p className="text-xs text-red-600 flex items-center">
                            <span className="inline-block w-2 h-2 bg-red-500 rounded-full mr-2"></span>
                            You have active loans that need to be settled first
                          </p>
                        )}
                      </div>
                    )}
